...
foundit:
	...
	case M_TEXT:
		...
		minfo.BytesPerScanLine = host_to_le16(mblock->swidth*2/mblock->cwidth);
		minfo.NumberOfPlanes = 0x4;
		minfo.BitsPerPixel = 4u;
	...
	int pages = 0;
	// const auto pages = (vga.vmemsize / pageSize)-1;
	...
	if (mblock->type==M_TEXT) {
		...
		minfo.XResolution = host_to_le16(mblock->swidth/mblock->cwidth);
		minfo.YResolution = host_to_le16(mblock->sheight/mblock->cheight);
	} else {
...
Bit8u VESA_ScanLineLength_24bit(Bit8u subcall,Bit16u val, Bit16u & bytes,Bit16u & pixels,Bit16u & lines) {
        Bit8u bpp = 3;
		switch (subcall) {
...
Bit8u VESA_ScanLineLength(Bit8u subcall,Bit16u val, Bit16u & bytes,Bit16u & pixels,Bit16u & lines) {
	...
	case M_LIN24:
		// pixels_per_offset = 3;
		return VESA_ScanLineLength_24bit(subcall, val, bytes, pixels, lines);
		break;
	case M_LIN32:
		pixels_per_offset = 2;
		break;
...
//TODO Maybe add normal vga modes too, but only seems to complicate things
	...
	int10.rom.used += (Bit16u)CALLBACK_Setup(callback.pmStart, VESA_PMSetStart, /*CB_VESA_START*/ CB_VESA_PM, PhysMake(0xc000,int10.rom.used), "VESA PM Set Start");
	/* PM Set Palette call */
