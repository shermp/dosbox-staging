...
static void composite_init(Section *sec)
{
	...
	is_composite_new_era = (std::string(conf->Get_string("era")) == "new");
	...
	if (state == "on")
		LOG_MSG("COMPOSITE: %s-era enabled with hue %d%%, saturation %d%%,"
		        ...
		        is_composite_new_era ? "New" : "Old", get_percent_from_hue(),
		        get_percent_from_saturation(), get_percent_from_contrast(),
		        get_percent_from_brightness(), get_percent_from_convergence());
}
...
static void composite_settings(Section_prop &secprop)
{
	...
	const char *eras[] = {"old", "new", 0};
	str_prop = secprop.Add_string("era", when_idle, "old");
	str_prop->Set_values(eras);
	str_prop->Set_help("Era of composite technology: 'new' is more vivid than 'old'.");
...
{
	assert(conf);
	Section_prop *sec = conf->AddSection_prop("composite", &composite_init, true);
	assert(sec);
