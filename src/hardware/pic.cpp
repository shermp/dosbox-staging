...
bool PIC_RunQueue(void) {
	...
	const auto index_nd = static_cast<float>(PIC_TickIndexND());
	InEventService = true;
	const auto cycles_max_float = static_cast<float>(CPU_CycleMax);
	while (pic_queue.next_entry && (pic_queue.next_entry->index * cycles_max_float <= index_nd)) {
		PICEntry *entry = pic_queue.next_entry;
		pic_queue.next_entry = entry->next;
	...
	if (pic_queue.next_entry) {
		Bits cycles = (Bits)(pic_queue.next_entry->index * cycles_max_float - index_nd);
