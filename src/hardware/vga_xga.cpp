...
void XGA_DrawWait(uint32_t val, io_width_t width)
{
	...
	case 2: /* Rectangle */
		...
		case 0x00: /* FOREMIX always used */
			...
			*/
			...
			case 0x20 | M_LIN8: // 16 bit
				...
				for (uint8_t i = 0; i < static_cast<uint8_t>(width); ++i) {
			...
			case 0x40 | M_LIN8: // 32 bit
				...
				break;
			case (0x20 | M_LIN32):
				if (width != io_width_t::dword) { // Win 3.11 864 'hack?'
		...
		case 0x02: // Data from PIX_TRANS selects the mix
			...
			for (Bitu k = 0; k < chunks; k++) { // chunks counter
				...
				for (Bitu n = 0; n < chunksize; ++n) { // pixels
					...
					// don't shadow the parent mixmode
					const uint32_t mix = (val & mask) ? xga.foremix : xga.backmix;
					switch ((mix >> 5) & 0x03) {
					case 0x00: // Src is background color
						srcval = xga.backcolor;
						break;
					case 0x01: // Src is foreground color
						...
						break;
					default:
						LOG_MSG("XGA: DrawBlitWait: Unsupported src %x", (mix >> 5) & 0x03);
						srcval = 0;
						break;
					}
					DrawWaitSub(mix, srcval);
		...
		default: LOG_MSG("XGA: DrawBlitWait: Unhandled mixmode: %d", mixmode); break;
		} // mixmode switch
		break;
	default: LOG_MSG("XGA: Unhandled draw command %x", xga.waitcmd.cmd); break;
	}
}
