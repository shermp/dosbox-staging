...
using io_port_t = uint16_t; // DOS supports 16-bit port addresses
using io_val_t = uint32_t; // DOS ports supports transactions up to 32 bits
...
inline void IO_Write(io_port_t port, uint8_t val)
{
	IO_WriteB(port,val);
}
inline uint8_t IO_Read(io_port_t port)
{
